...
export const handleTouchMove: (props: HandleEventMoveProps) => TouchEventHandler<HTMLDivElement> =
  ...
  (e) => {
    ...
    if (onHandling) {
      topCoordRef.current = e.touches[0].clientY;
      const target = e.target as HTMLDivElement;
      target.parentElement?.style.setProperty('top', `${e.touches[0].clientY + modifyRef.current}px`);
...
// export const handlePopUpTouchMove: ({
//   setTabState,
//   setCameraState,
//   smoothLoopId,
//   cameraState,
//   tabState,
// }: HandlePopUpTouchMoveProps) => TouchEventHandler<HTMLDivElement> =
//   ({ setTabState, smoothLoopId, tabState }) =>
//   (e) => {
